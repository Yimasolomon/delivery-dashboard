...
type updateDeliveryForm struct {
	CustomerID         string
	DriverID           string
	PickupAddress      string
	DeliveryAddress    string
...
func (f updateDeliveryForm) toModel(existing model.Delivery) (model.Delivery, error) {
	customerID, err := strconv.ParseInt(f.CustomerID, 10, 64)
	if err != nil || customerID <= 0 {
		return model.Delivery{}, fmt.Errorf("invalid customer ID")
	}
	...
	existing.PickupAddress = strings.TrimSpace(f.PickupAddress)
	existing.DeliveryAddress = strings.TrimSpace(f.DeliveryAddress)
	existing.PackageDescription = strings.TrimSpace(f.PackageDescription)
	existing.Quantity = quantity
	existing.Weight = weight
	existing.DeliveryDate = deliveryDate
	existing.EstimatedDelivery = estimatedDelivery
	existing.Notes = strings.TrimSpace(f.Notes)
